...
//     const twoFASecret = "CR42V266O2326V2XIHZ6U72TL5U33O4J";
...
//       password: "Admin@123",
//       twoFA: code,
//       vendor_code: "FA341709_U",
//       api_secret: "c046bb96c4be10e9fb3eff8b90d8ee2d",
//       imei: "abc1234",
//     };
...
const LiveData = ({ watchlist }) => { // Accept watchlist as a prop
  ...
  useEffect(() => {
    ...
    const twoFASecret = "63E25C5RTI5BPWH364WZC5VQ6OLMW427";
    const code = authenticator.generate(twoFASecret);
    console.log("Generated 2FA Code:", code);
  ...
  const open = () => {
    if (watchlist.length > 0) {
      const instrumentsString = watchlist.map(item => `${item.exchange}|${item.token}`).join('#');
      api.subscribe(instrumentsString, "depth");
      console.log("Subscribing to Depth:", instrumentsString);
    } else {
      console.warn("Watchlist is empty. No subscriptions made.");
    }
  };

  const receiveDepth = (data) => {
    console.log("Depth Data Received:", data); // Log the incoming data
    setLiveData((prevData) => {
      const existingIndex = prevData.findIndex(item => item.tk === data.tk);
      if (existingIndex > -1) {
        // Update existing subscription
