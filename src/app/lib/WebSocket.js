// Using browser WebSocket; do not import Node 'ws' in client bundles.
...
let WebSocketClient = function (cred, params) {
     ...
     this.connect = function (params, callbacks) {
         return new Promise((resolve, reject) => {
             ...
             // Initialize WebSocket without subprotocols
     ...
     this.set_callbacks = function (callbacks) {
         if (callbacks) { 
             ...
             if (callbacks.depth !== undefined) { 
                this.on('depth', callbacks.depth);
            }
         }
     };
    ...
    this.unsubscribe = function (instrument, feedtype) {
        let values = {};
        if (feedtype === 'depth') {
            values['t'] = 'ud'; // 'u' represents unsubscribe for depth
        } else {
            values['t'] = 'u'; // 'u' represents unsubscribe for regular
        }
        values['k'] = instrument;
